...
below. Per this change's non-goals, `fix` findings are **logged and handed to a
follow-up delta**, not patched here (a late edit to a tested TCB service is itself a
risk); each is Low and, for the non-production demo, an acceptable interim residual.
...
| 2 | **Every state-changing inter-service call authenticated** | **pass** (+ SR-02) | P — broker no-auth→401, wrong-aud→403, garbage→401; tools-api no-token→401; rag no-token→401. C — bearer+JWKS+`aud` check in each service's `validation.py`. SR-02 logs the `/ciba/delegate` gap. |
...
| 11 | **External surfaces validate input & leak nothing on error** | **pass** (+ SR-01, SR-03) | P — rag missing-field→400 generic; broker error body carries no stack trace/secret. SR-01 (exception text), SR-03 (size cap) logged |
...
- **C approval** — `binding_message` is `apr-`+`token_hex(12)` (`^apr-[0-9a-f]{24}$`, hex-only, injection-safe); UI renders **service-held** payload only (`app.py:114`); `consume` verifies action-token, `status==approved`, CIBA-token subject == owner, payload hash, and atomic single-use (`app.py:146`). T — `test_human_approval.py`, `test_reactive_approval.py`. Gap: SR-02.
...
- **SR-02** — no test asserts `/ciba/delegate` rejects a non-Keycloak caller (because it currently doesn't). File the test with the fix.
- **SR-01** — no test asserts error bodies are free of exception text. Add a probe-style negative test.

## Disposition summary

12/12 baseline requirements have a recorded disposition; **all pass**, with **3 Low
findings** (SR-01, SR-02 → `fix`; SR-03 → `accepted-residual`) and the residual-risk
register above. Control weaknesses (SR-02, the trusted-code assumptions) are handed to

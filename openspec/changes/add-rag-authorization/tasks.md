...
- [ ] 1.1 Verify pgvector in the running Postgres image: `CREATE EXTENSION vector`, a
  `vector` column, and a cosine-distance top-K query all work (or switch to a
  pgvector-enabled image tag). Record in design.
- [ ] 1.2 Prove the local deterministic embedder + a seeded corpus makes a chosen
...
- [ ] 1.3 Prove an OpenFGA `batch_check` for `document:{id} viewer user:{sub}` over
...
- [ ] 1.4 Confirm an `mcp-server → aud=rag-server` RFC 8693 exchange preserves `sub`
...
- [ ] 2.1 Add a bearer-capable `rag-server` resource client to `realm-export.json`,
...
- [ ] 2.2 Grant `mcp-server` standard token-exchange permission for `aud=rag-server`
...
- [ ] 3.1 Scaffold `services/rag/` mirroring `services/token-broker/` (FastAPI, OTel
...
- [ ] 5.2 `rag_search` MCP tool in `services/mcp/tools.py`: exchanges the inbound
...
- [ ] 6.1 `test_rag_authorization.py`: authorized user retrieves a protected doc;
  unauthorized user does not; a foreign-audience / no-identity call returns no chunks.
- [ ] 6.2 **Adversarial leakage** test: the protected doc is the top embedding hit for
...
- [ ] 6.3 `rag_search` through MCP: end-user `sub` preserved, only that user's chunks
  returned, inbound MCP token not forwarded downstream.

## 7. Verify + wrap

- [ ] 7.1 Add a RAG row to the Grafana dashboard (retrieval count stat + live
  `rag_audit` logs); confirm rendering in a real browser (screenshot).
- [ ] 7.2 Drive the full flow and **look**: through MCP `rag_search`, alice retrieves
  ...
  `rag_audit` in Loki.
- [ ] 7.3 `docs/threat-model.md`: add the "RAG authorization (Flow D, M5)" section —
...
- [ ] 7.4 Clean-slate `down -v && up` reproduces the milestone; full smoke suite green

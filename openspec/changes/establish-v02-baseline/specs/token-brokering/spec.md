...
Every token issuance and every denial SHALL be audit-logged with `{user, agent (azp), provider, scopes, ttl}` and a correlation ID linking related broker, Keycloak, and approval events.

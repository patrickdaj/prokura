...
## Resolved by the M5 spike (spike/rag/rag_spike.py; run against the live stack)

Mirroring M0/M2/M3/M4, M5 opened with a light spike. Findings:

- **pgvector NOT in `postgres:17-alpine`** (`CREATE EXTENSION vector` → "extension
  not available"). The **`pgvector/pgvector:pg17`** image bundles pgvector 0.8.6;
  `CREATE EXTENSION vector`, a `vector(N)` column, and cosine top-K (`<=>`) all work
  with correct ranking. → the build switches the Postgres image tag; no new container.
- **Offline embedder + seeded corpus makes the protected doc the top hit.** A
  deterministic blake2b hashing bag-of-words vectorizer (DIM=256, L2-normalized,
  cosine) over the seeded corpus ranks the protected `secret-roadmap` doc #1
  (cos≈0.68) for the crafted acquisition-timeline query, decisively above the next
  hit (cos≈0.09). Embeddings are byte-identical across runs (blake2b bypasses hash
  randomization). No network, no secret.
- **OpenFGA `batch_check` as the end user filters correctly.** The real
  `/stores/{id}/batch-check` endpoint (per-check `correlation_id`) with subject
  `user:{end-user}` over the candidate set: the protected doc is allowed for alice
  (viewer) and denied for bob (non-viewer); a `user:*` public doc is visible to both;
  an owner-only doc stays private. Model unchanged (`type document` already ships).
- **`mcp-server → aud=rag-server` RFC 8693 exchange preserves the subject.** Confirmed
  live against the new `rag-audience` scope / `rag-server` client: the exchanged token
  carries `aud=rag-server`, `azp=mcp-server`, and the same `sub` as the inbound MCP
  token — re-confirming the M4 finding for the new audience.

**End-user identity = `preferred_username`, not the raw `sub`.** The spike found the
token `sub` is a Keycloak UUID while `preferred_username` is the stable username
(`alice`). The broker and the whole existing FGA model key on the username
(`grant:alice/acme`, `user:alice`), and the Drive-shaped manifest names owners/viewers
by username. So the retriever's FGA subject is `user:{preferred_username}` — the
concrete, cross-service-consistent reading of the spec's `user:{sub}`. Recorded here

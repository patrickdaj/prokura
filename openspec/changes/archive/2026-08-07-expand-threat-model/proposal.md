...
- **A findings-narrative threat-modeling blog** (`docs/blog/`, milestone-series style): a reader-facing post that walks the model's headline results — the highest-value attack paths (token-broker compromise, `can_use` tuple forgery, approval spoofing/replay, RAG confused-deputy) and, for each, the control that breaks it or the residual we accept and why. It is the "here's what the threat model found and how it's defended" story, distinct from `docs/threat-model.md` (the rigorous reference); it cross-links the model and the milestone build-log blogs.
...
- **Docs**: rewrites/expands `docs/threat-model.md` from working draft to complete model, and adds a findings-narrative threat-modeling blog under `docs/blog/`.

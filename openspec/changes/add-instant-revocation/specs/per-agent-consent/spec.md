...
- **THEN** the same tuple delete, deny-list entry, Keycloak revocation, signal, and audit event occur as when revoking from the broker's consent surface, and the owner is the bearer's verified subject
...
#### Scenario: Revoked agent cannot re-acquire authority
- **WHEN** a revoked agent attempts to refresh its offline token and re-exchange for that user
- **THEN** no fresh user-bound token is issued, so re-consent (not a stale token) is required to act again

...
The required format, coverage, and traceability of the `docs/adr/` corpus — ADR structure, one-ADR-per-decision completeness against the known decision set (SPEC-REVIEW F1–F9 / Q1–Q7 + locked choices), and a citation back to the OpenSpec artifact or SPEC-REVIEW ID that is each decision's source of truth. Realized in [docs/adr/](../../../docs/adr/).

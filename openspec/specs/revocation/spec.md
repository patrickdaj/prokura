...
The kill switch (v1 thesis #3, M9). Answers "how fast can you make an agent stop?" with a
measured number: a per-grant revoke deletes the `can_use` tuple, writes a broker deny-list
entry (a propagation-free stop checked on every hand-out), and emits a CAEP Security Event
Token — instant and scoped. The agent-wide kill additionally revokes the agent client's
Keycloak sessions/offline tokens so it cannot re-acquire authority. The only window it cannot
close — a provider token already handed out — is bounded and reported honestly.
